...
//import org.slf4j.Logger;		// fancy print function
...
public class PageFetcher extends Configurable {
  ...
  private void doBasicLogin(BasicAuthInfo authInfo) {
	//System.out.println("BASIC authentication for: " + authInfo.getLoginTarget());
  ...
  private void doNtLogin(NtAuthInfo authInfo) {
    // System.out.println("NT authentication for: " + authInfo.getLoginTarget());
    HttpHost targetHost = new HttpHost(authInfo.getHost(), authInfo.getPort(), authInfo.getProtocol());
    CredentialsProvider credsProvider = new BasicCredentialsProvider();
    try {
      credsProvider.setCredentials(new AuthScope(targetHost.getHostName(), targetHost.getPort()),
    ...
    //	 System.out.println ("Error creating NT credentials "+ e);
    }
    httpClient = HttpClients.custom().setDefaultCredentialsProvider(credsProvider).build();
  }
  ...
  private void doFormLogin(FormAuthInfo authInfo) {
	//System.out.println("FORM authentication for: " + authInfo.getLoginTarget());
    String fullUri =
