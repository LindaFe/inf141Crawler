      ...
      if (statusCode < 200 || statusCode > 299) { // Not 2XX: 2XX status codes indicate success
        if (statusCode == HttpStatus.SC_MOVED_PERMANENTLY || statusCode == HttpStatus.SC_MOVED_TEMPORARILY ||
          ...
          if (myController.getConfig().isFollowRedirects()) {
            ...
            if (movedToUrl == null) {
            	throw new Exception("Unexpected error, URL: " + curURL + " is redirected to NOTHING");
            }
            page.setRedirectedToUrl(movedToUrl);

            int newDocId = docIdServer.getDocId(movedToUrl);
            if (newDocId > 0) {
            	throw new Exception ("Redirect page: " + curURL + " is already seen");
            }
        ...
        if (!curURL.getURL().equals(fetchResult.getFetchedUrl())) {
          if (docIdServer.isSeenBefore(fetchResult.getFetchedUrl())) {
            throw new Exception("Redirect page: " + curURL + " has already been seen");
          }
